...
def view_cluster_vx():
    ...
    for partition in config.PARTITIONS:
        csv_path = Path(config.ROOT_DATA1) / partition / 'camera_data.csv'
        if not csv_path.exists():
            print(f"[{partition}] No se encontró camera_data.csv en: {csv_path}")
            continue

        print(f"[{partition}] Cargando {csv_path}")
        ...
        for cluster_id in range(n_clusters):
            ...
            # Representante del cluster (punto más cercano al centroide)
        ...
        ax.set_title(f"[{partition}] Grupos ({n_clusters} Clusters) - Solo Representantes", fontsize=14)
        ax.set_xlabel("X (Este)"); ax.set_ylabel("-Y (Norte)"); ax.set_zlabel("Z (Altura)")
        ax.legend(handles=legend_elements, title="Grupos de Coincidencia",
                  loc='center left', bbox_to_anchor=(1.07, 0.5))
